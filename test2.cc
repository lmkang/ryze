...
std::shared_ptr<ModuleEmbedderData> GetModuleDataFromContext(
  ...
  v8::Local<v8::Value> module_data = context->GetEmbedderData(0);
...
void Load(const v8::FunctionCallbackInfo<v8::Value>& args) {
  for (int i = 0; i < args.Length(); i++) {
    ...
    v8::ScriptCompiler::Source _source(source, origin);
    v8::Local<v8::Module> module;
    if(v8::ScriptCompiler::CompileModule(isolate, &_source).ToLocal(&module)) {
